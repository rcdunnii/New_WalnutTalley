...
function list() {
    $(".content").mCustomScrollbar({
        ...
        advanced: {
         ...
         // make email address a mailto:address on click
function sendMyEmail(recipient) {
window.open(location.href="mailto:" + recipient, "_blank");
}
...
jqxhr = $.ajax({ 
    ...
    window.location.replace("https://" + thisHost + "/mainChoice?main=listNuts"); 
...
jqxhr = $.ajax({ 
 ...
 // $("#formalNames").val(data.FormalNames);
    ...
    if (data.Updated) {$("#updated").val(data.Updated);}
    $(".list").hide();
    $(".edit").show();		
}).fail ( function (jqxhr, status, error) {
